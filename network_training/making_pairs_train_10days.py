...
def making_pairs(image,label):
    for i in range(0,len(image)-9):
        temp=np.array(label[i+1:i+9])
        if np.all(temp=='0'):
            a=random.randint(1,2)
            if a==1 or a==2:
                print("%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s" %(image[i], image[i + 1],image[i + 2],image[i + 3],image[i + 4],image[i + 5],image[i + 6],image[i + 7],image[i + 8],image[i + 9], label[i + 1], label[i + 2], label[i + 3], label[i + 4],label[i + 5],label[i + 6],label[i + 7],label[i + 8],label[i + 9]))
        else:
            print("%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s" % (
            image[i], image[i + 1], image[i + 2], image[i + 3], image[i + 4], image[i + 5], image[i + 6], image[i + 7],
            image[i + 8], image[i + 9], label[i + 1], label[i + 2], label[i + 3], label[i + 4], label[i + 5],
            label[i + 6], label[i + 7], label[i + 8], label[i + 9]))
