...
from dataset_EQ_distribution_7days import *
import parameters
...
#model = resnet18(num_img=7,num_classes=6)
model = resnet34(num_img=7,num_classes=6)
#model=resnet101(num_img=7,num_classes=6)
if args.cuda:
    model.cuda()
...
N_ground_truth=0
test_loss=0
output=np.zeros(len(dataset)+6)
weight=np.zeros(len(dataset)+6)
weight[0]=1
for i, (x) in enumerate(train_loader):
    ...
    with torch.no_grad():
        ...
        y=y.flatten()
    output[i+1:i+7]+=np.array(y)
    weight[i+1:i+7]+=np.ones(6)
