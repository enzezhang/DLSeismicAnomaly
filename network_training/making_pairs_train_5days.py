...
def making_pairs(image,label):
    for i in range(0,len(image)-4):
        temp=np.array(label[i+1:i+4])
        if np.all(temp=='0'):
            a=random.randint(1,3)
            if a==1 or a==2:
                print("%s %s %s %s %s %s %s %s %s" %(image[i], image[i + 1],image[i + 2],image[i + 3],image[i + 4], label[i + 1], label[i + 2], label[i + 3], label[i + 4]))
        else:
            print("%s %s %s %s %s %s %s %s %s" % (
            image[i], image[i + 1], image[i + 2], image[i + 3], image[i + 4], label[i + 1], label[i + 2], label[i + 3],
            label[i + 4]))
