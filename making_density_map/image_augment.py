...
def rotate2(image_np, save_dir, input_filename,degree=[180]):
    file_basename = os.path.basename(input_filename)
    basename = os.path.splitext(file_basename)[0]
    ext = os.path.splitext(file_basename)[1]

    for angle in degree:
        images_r=ndimage.rotate(image_np, angle)
        save_path = os.path.join(save_dir, basename + '_R' + str(angle) + ext)
        io.imsave(save_path, images_r)
...
def move(image_np,save_dir,input_filename,dis_h,dis_v):
    file_basename = os.path.basename(input_filename)
    basename = os.path.splitext(file_basename)[0]
    ext = os.path.splitext(file_basename)[1]
    for value in dis_h:
        image_l= np.zeros(image_np.shape)
        image_r = np.zeros(image_np.shape)
        image_l[:,0:image_np.shape[1] - value] = image_np[:,value:]
        image_r[:,value:] = image_np[:,0:image_np.shape[1] - value]
        save_path = os.path.join(save_dir, basename + '_Left' + str(value) + ext)
        io.imsave(save_path, image_l)
        save_path = os.path.join(save_dir, basename + '_Right' + str(value) + ext)
        io.imsave(save_path, image_r)

    for value in dis_v:
        image_u = np.zeros(image_np.shape)
        image_d = np.zeros(image_np.shape)
        image_u[0:image_np.shape[0] - value, :] = image_np[value:, :]
        image_d[value:, :] = image_np[0:image_np.shape[0] - value, :]
        save_path = os.path.join(save_dir, basename + '_Up' + str(value) + ext)
        io.imsave(save_path, image_u)
        save_path = os.path.join(save_dir, basename + '_Down' + str(value) + ext)
        io.imsave(save_path, image_d)

    return True

def move2(image_np,save_dir,input_filename):
    file_basename = os.path.basename(input_filename)
    basename = os.path.splitext(file_basename)[0]
    ext = os.path.splitext(file_basename)[1]
    [y_index,x_index]=np.where(image_np==image_np.max())
    if y_index.size==1:
        print('only one maximum')
    else:
        print("mulitple maximum")
        return False
    y_index=int(y_index)
    x_index=int(x_index)
    ...
    image_l[:,0:image_np.shape[1] - x_index+a] = image_np[:,x_index-a:]
    save_path = os.path.join(save_dir, basename + '_Left' + ext)
    io.imsave(save_path, image_l)

    a = random.randint(1, 7)
    if x_index+a+1<image_np.shape[1]:
        image_r[:,image_np.shape[1]-x_index-a:] = image_np[:,0:x_index+a]
        save_path = os.path.join(save_dir, basename + '_Right' + ext)
        io.imsave(save_path, image_r)
    ...
    image_d = np.zeros(image_np.shape)
    a = random.randint(1, 7)
    image_u[0:image_np.shape[0] - y_index+a, :] = image_np[y_index-a:, :]
    save_path = os.path.join(save_dir, basename + '_Up'  + ext)
    io.imsave(save_path, image_u)

    a = random.randint(1, 7)
    if y_index+a+1<image_np.shape[0]:
        image_d[image_np.shape[0]-y_index-a:,:] = image_np[0:y_index+a, :]
        save_path = os.path.join(save_dir, basename + '_Down' + ext)
        io.imsave(save_path, image_d)
...
def image_augment(img_path,save_dir,is_groud_true):
    ...
    if Flip(img_test, save_dir, basename) is False:
        return False
    if rotate2(img_test, save_dir, basename, degree=[45,90,135,180,225,270,315]) is False:   #45, 90, 135
        return False
    # move2(img_test,save_dir,basename)
    if move(img_test, save_dir, basename, dis_h=[5,10,15,20,25],dis_v=[5,10]) is False:   #45, 90, 135
        return False
